...
class Test:
    ...
    def __init__(self):
        ...
        self.test(p.get_board(), [[1, 2], [3, 0]])
        p = create_tile_puzzle(3, 3)
        self.test(p.get_board(), [[1, 2, 3], [4, 5, 6], [7, 8, 0]])
        self.test(p.perform_move("up"), True)
        self.test(p.get_board(), [[1, 2, 3], [4, 5, 0], [7, 8, 6]])
        p = TilePuzzle([[1, 2], [3, 0]])
        self.test(p.is_solved(), True)
        p = TilePuzzle([[0, 1], [3, 2]])
        self.test(p.is_solved(), False)
        p = create_tile_puzzle(3, 3)
        p2 = p.copy()
        self.test(p.get_board(), p2.get_board())
        self.test(p.calc_distance(), 0)
        b = [[4, 1, 2], [0, 5, 3], [7, 8, 6]]
        p = TilePuzzle(b)
        print p.find_solution_a_star()

    def test(self, t, expect, desc=""):
        if t == expect:
            self._test_score += 1
            print "passed", desc
        else:
            print("unexpected:", t, "!=", expect)
            print("test progress:", self._test_score)
